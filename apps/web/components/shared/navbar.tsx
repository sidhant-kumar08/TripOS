...
import { Compass, LogOut, LayoutDashboard, Menu, X, ArrowRight, User as UserIcon, Calendar, DollarSign, FileText } from 'lucide-react';
    ...
    <div className="sticky top-3 z-50 mx-auto w-full max-w-6xl px-3 sm:px-6">
      ...
      >
        <div className="flex items-center justify-between">
          ...
          {/* Desktop Navigation Links (Only shown when inside a trip or on the public landing page) */}
          {(currentTripId || isLanding) && (
                ...
                <>
                  <Link
                    ...
                    className={`rounded-full px-3 py-1 text-xs font-semibold transition-all flex items-center gap-1.5 ${
                      pathname === `/trips/${currentTripId}`
                        ? 'bg-white text-indigo-600 shadow-sm dark:bg-slate-800 dark:text-indigo-400 font-bold'
                  ...
                  <Link
                    ...
                    className={`rounded-full px-3 py-1 text-xs font-semibold transition-all flex items-center gap-1.5 ${
                      pathname === `/trips/${currentTripId}/itinerary`
                        ? 'bg-white text-indigo-600 shadow-sm dark:bg-slate-800 dark:text-indigo-400 font-bold'
                        : 'text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white hover:bg-white/80 dark:hover:bg-slate-800/80'
                    }`}
                  >
                  ...
                  <Link
                    ...
                    className={`rounded-full px-3 py-1 text-xs font-semibold transition-all flex items-center gap-1.5 ${
                      pathname === `/trips/${currentTripId}/expenses`
                        ? 'bg-white text-indigo-600 shadow-sm dark:bg-slate-800 dark:text-indigo-400 font-bold'
                        : 'text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white hover:bg-white/80 dark:hover:bg-slate-800/80'
                    }`}
                  >
                  ...
                  <Link
                    href={`/trips/${currentTripId}/vault`}
                    className={`rounded-full px-3 py-1 text-xs font-semibold transition-all flex items-center gap-1.5 ${
                      pathname === `/trips/${currentTripId}/vault`
                        ? 'bg-white text-indigo-600 shadow-sm dark:bg-slate-800 dark:text-indigo-400 font-bold'
                        : 'text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white hover:bg-white/80 dark:hover:bg-slate-800/80'
                    }`}
                  >
            ...
            {currentTripId ? (
              <div className="flex flex-col space-y-1">
                <Link
                  ...
                  className="rounded-xl px-3.5 py-2 text-xs font-semibold text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white flex items-center gap-2"
                >
                ...
                <Link
                  href={`/trips/${currentTripId}`}
                  onClick={() => setMobileMenuOpen(false)}
                  className={`rounded-xl px-3.5 py-2 text-xs font-semibold flex items-center gap-2.5 transition ${
                    pathname === `/trips/${currentTripId}`
                      ? 'bg-indigo-50 text-indigo-600 dark:bg-indigo-950/80 dark:text-indigo-300 font-bold'
