...
import { GoogleIcon } from '@/components/auth/social-auth';
...
function GoogleCallbackContent() {
  ...
  React.useEffect(() => {
    if (errorParam) {
      setStatus('error');
      setErrorMessage(errorDescription || `Google authentication was cancelled or failed: ${errorParam}`);
      return;
    }
      ...
      try {
        ...
        setErrorMessage(
          err.response?.data?.message ||
            'Failed to complete Google Sign-In. Please check that your Google OAuth credentials are configured correctly.'
        );
      }
    };
        ...
        {status === 'loading' && (
          <div className="space-y-3">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Verifying your Google credentials and preparing your trip workspace...
            </p>
          </div>
        )}
        ...
        {status === 'error' && (
          <div className="space-y-4">
            <div className="flex items-center justify-center gap-2 text-red-600 dark:text-red-400">
              ...
              <h2 className="text-lg font-bold">Google Sign-In Error</h2>
            </div>
